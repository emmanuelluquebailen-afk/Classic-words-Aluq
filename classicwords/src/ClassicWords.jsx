...
function findAIMove(board,rack,dict,isFirst,diffKey,LV){
  ...
  function tryPlace(word,sr,sc,DR,DC){
    if(Date.now()>deadline)return; // deadline atteinte → stop
    ...
    moves.push({placed,scored,total,strategicBonus:0});
  }
  ...
  if(isFirst){
    ...
    // Algo ancres : pour chaque lettre du plateau, générer des mots qui l'utilisent
      ...
      for(const word of words){
        ...
        for(let wi=0;wi<wl;wi++){
          ...
          for(const[ar,ac]of positions){
            ...
            if(!isOds&&moves.length>=maxMoves)break;
          }
          if(!isOds&&moves.length>=maxMoves)break;
        }
        if(!isOds&&moves.length>=maxMoves)break;
    ...
    // Si peu de coups, chercher aussi avec le chevalet seul en adjacence
    if(moves.length<5&&Date.now()<deadline){
      ...
      for(const word of rackWords){
        ...
        for(const[DR,DC]of[[0,1],[1,0]]){
          for(let r=0;r<SIZE;r++){
            for(let c=0;c<SIZE-(wl-1)*(1-DR);c++){
              if(Date.now()>deadline)break;
              let adj=false;
  ...
  if(diffKey==='ods')return moves[0]; // ODS : meilleur coup dans le temps imparti
...
// Fallback ODS : si l'IA ne trouve aucun coup, cherche le premier mot de 2-3 lettres jouable
function findFallbackMove(board,rack,dict,isFirst,LV){
  const rackLetters=rack.map(t=>t.letter);
  const ui2={errMin:'',errAlign:'',errGap:'',errCenter:'',errTouch:''};
  // Mots courts du chevalet seul
  function wordsShort(avail){
    const res=new Set();
    function h(cur,rem){
      if(cur.length>=2&&dict.has(cur.join('')))res.add(cur.join(''));
      if(cur.length>=3||!rem.length)return;
      for(let i=0;i<rem.length;i++){const r=[...rem];r.splice(i,1);h([...cur,rem[i]],r);}
    }
    h([],[...avail]);return res;
  }
  const words=wordsShort(rackLetters);
  if(!words.size)return null;
  for(const word of words){
    const wl=word.length;
    if(isFirst){
      for(let off=0;off<wl;off++){
        const placed={};const rc=[...rackLetters];let ok=true;
        for(let i=0;i<wl;i++){
          const r=7,c=7-off+i;if(c<0||c>=SIZE){ok=false;break;}
          const ri=rc.indexOf(word[i]);if(ri<0){ok=false;break;}
          rc.splice(ri,1);placed[`${r},${c}`]={letter:word[i]};
        }
        if(ok&&placed['7,7']){
          const ws=findWords(board,placed);
          if(ws.length&&ws.every(w2=>dict.has(w2.word))){
            const{total,scored}=calcScore(board,placed,ws,LV);
            return{placed,scored,total};
          }
        }
      }
    }else{
      for(let r=0;r<SIZE;r++)for(let c=0;c<SIZE;c++){
        for(const[DR,DC]of[[0,1],[1,0]]){
          if(r+(wl-1)*DR>=SIZE||c+(wl-1)*DC>=SIZE)continue;
          const placed={};const rc=[...rackLetters];let ok=true;let adj=false;
          for(let i=0;i<wl;i++){
            const rr=r+i*DR,cc=c+i*DC;
            if(board[rr][cc]?.letter){if(board[rr][cc].letter!==word[i]){ok=false;break;}adj=true;}
            else{const ri=rc.indexOf(word[i]);if(ri<0){ok=false;break;}rc.splice(ri,1);placed[`${rr},${cc}`]={letter:word[i]};}
          }
          if(!ok||!Object.keys(placed).length)continue;
          if(!adj){
            const hasAdj=Object.keys(placed).some(k=>{const[pr,pc2]=k.split(',').map(Number);return[[-1,0],[1,0],[0,-1],[0,1]].some(([dr,dc2])=>board[pr+dr]?.[pc2+dc2]);});
            if(!hasAdj)continue;
          }
          if(!validatePlacement(board,placed,false,ui2).ok)continue;
          const ws=findWords(board,placed);
          if(ws.length&&ws.every(w2=>dict.has(w2.word))){
            const{total,scored}=calcScore(board,placed,ws,LV);
            return{placed,scored,total};
...
function LangPicker({onPick,theme}){
  ...
  const LANGS=[
    {code:'EN',flag:'🇬🇧',name:'ENGLISH',badge:'Mode Famille',badgeColor:'#2E7D32',desc:'Dictionnaire anglais · Définitions en français'},
    {code:'FR',flag:'🇫🇷',name:'FRANÇAIS',badge:'Mode Famille',badgeColor:'#1565C0',desc:'Dictionnaire français · Définitions en anglais'},
    {code:'FR_ODS',flag:'🏆',name:'ODS Officiel',badge:'Mode Puriste',badgeColor:'#B8860B',desc:'416 000 mots · Scrabble officiel FISF'},
  ];
  return(
    <div style={{minHeight:'100dvh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',background:T.bgGrad,fontFamily:FF,color:T.text,padding:'24px',gap:'24px'}}>
      <div style={{textAlign:'center'}}>
        <img src="/icon.png" alt="WORDAQ" style={{width:'120px',height:'120px',borderRadius:'24px',boxShadow:'0 8px 32px rgba(0,0,0,0.5)',display:'block',margin:'0 auto 12px'}}/>
        <p style={{margin:0,fontSize:'11px',color:'rgba(255,255,255,0.8)',letterSpacing:'2px',fontWeight:'600'}}>CHOISISSEZ VOTRE MODE</p>
      </div>
      <div style={{display:'flex',flexDirection:'column',gap:'10px',width:'100%',maxWidth:'320px'}}>
        {LANGS.map(({code,flag,name,badge,badgeColor,desc})=>(
          <button key={code} onClick={()=>onPick(code)} style={{
            ...
            <span style={{fontSize:'36px',flexShrink:0}}>{flag}</span>
            <div style={{flex:1,minWidth:0}}>
              <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'3px'}}>
                <span style={{fontSize:'14px',fontWeight:'900',color:'#1A1A2E',letterSpacing:'1px'}}>{name}</span>
                <span style={{fontSize:'9px',fontWeight:'700',color:'#FFF',background:badgeColor,borderRadius:'4px',padding:'2px 6px',flexShrink:0}}>{badge}</span>
              </div>
              <span style={{fontSize:'10px',color:'#555',lineHeight:1.4}}>{desc}</span>
            </div>
...
function AluQWordsInner(){
  ...
  if(screen==='diff')return<DiffPicker lang={lang} onPick={pickDiff} onBack={()=>setScreen('lang')} onStats={()=>setScreen('stats')} onAbout={()=>setScreen('about')} onMulti={()=>{if(!dict||prevLang!==lang){setDict(null);setDictErr(null);setScreen('dict_before_multi');}else setScreen('multi');}} onFeedback={()=>setScreen('feedback')} theme={theme} onTheme={setTheme}/>;
  ...
  // ECRAN NOIR FIX : dict chargé AVANT le lobby → plus de race condition
  // dict_before_multi : charge le dict puis va au lobby
  if(screen==='dict_before_multi'){
    if(dictErr)return(<div style={{minHeight:'100dvh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',background:THEMES[theme].bgGrad,color:THEMES[theme].text,fontFamily:FF,gap:'16px',padding:'32px',textAlign:'center'}}><p>⚠️ Erreur dictionnaire</p><button onClick={()=>setDictErr(null)} style={{padding:'10px 20px',background:'rgba(255,255,255,0.2)',border:'none',borderRadius:'8px',color:'#FFF',cursor:'pointer'}}>Réessayer</button></div>);
    return<DictLoader lang={lang} onLoaded={d=>{setDict(d);setPrevLang(lang);setScreen('multi');}} onError={setDictErr} theme={theme}/>;
  }
  if(screen==='multi')return<MultiplayerLobby lang={lang} diff={diff||'normal'} theme={theme} onBack={()=>setScreen('diff')} onStartGame={state=>{
    // Dict déjà chargé → transition directe, zéro risque de stale closure
    setMpState(state);
  ...
  // dict_multi supprimé — remplacé par dict_before_multi (chargement AVANT le lobby)
